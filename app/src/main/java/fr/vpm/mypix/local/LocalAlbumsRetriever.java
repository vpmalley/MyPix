...
public class LocalAlbumsRetriever {
  ...
  @NonNull
  public void getLocalAlbums(Context context, OnAlbumsRetrievedListener onAlbumsRetrievedListener) {
    try (Cursor localAlbums = context.getContentResolver().query(
      ...
      if (localAlbums != null) {
        ...
        onAlbumsRetrievedListener.onAlbumsRetrieved(new ArrayList<>(albumsById.values()));
      }
    }
    onAlbumsRetrievedListener.onAlbumsRetrieved(new ArrayList<>());
  }
    ...
    if (localAlbums.moveToFirst()) {
      while (localAlbums.moveToNext()) {
        ...
        if (currentAlbum == null || !albumsById.containsKey(bucketId)) {
          currentAlbum = new Album(bucketId, bucketName, null, Album.Source.LOCAL, -1);
  ...
  public interface OnAlbumsRetrievedListener {
    void onAlbumsRetrieved(final List<Album> albums);
  }
